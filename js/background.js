...
{
    chrome.notifications.create('Proprietaire', {type: 'basic', title: title, message: text, iconUrl:'images/red_pill.png', isClickable: false}, function(id){
        currentNotificationId = id;
        timer = setTimeout(function(){chrome.notifications.clear(id);}, time);
    });
    return;
}
...
function tabCallback(tabId, url, active)
{
    if(!url.startsWith('chrome://')) {
        ...
        if(!(tabId in lastDomain) || lastDomain[tabId][0] !== domain) {
            // Save URL as last URL and clear 'need to display' flag
            lastDomain[tabId] = [domain, false];
            // Get the owner
            ...
            if(owner) {
                ...
                if(active) {
                    displayNotification(owner, getDomainFromUrl(url), 3500);
                }
                else {
...
chrome.tabs.onActivated.addListener(function(activeInfo){
    ...
    if(activeInfo.tabId in lastDomain && lastDomain[activeInfo.tabId][1]) {
        displayNotification(getOwnerFromDomain(lastDomain[activeInfo.tabId][0]), lastDomain[activeInfo.tabId][0], 3500);
